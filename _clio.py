...
CLIO_ENDPOINTS = {"vnc-annotations-query": "json-annotations/VNC/neurons/query"}
...
def get_clio_token():
    ...
    if os.path.exists(TOKEN_CACHE_FILE):
        ...
        r = requests.post(
            CLIO_TOKEN_URL, headers={"Authorization": f"Bearer {user_token}"}
        )
...
def post(store: str, endpoint: str, json_payload: dict = None, str_payload: str = None):
    if json_payload:
        r = requests.post(
            clio_url(store, endpoint),
            json=json_payload,
            headers={"Authorization": f"Bearer {flyem_token}"},
        )
    else:
        r = requests.post(
            clio_url(store, endpoint),
